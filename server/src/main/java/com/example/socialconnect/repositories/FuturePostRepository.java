...
public interface FuturePostRepository extends CrudRepository<FuturePost, Integer> {
    ...
    @Query(
        nativeQuery = true,
        value = "INSERT INTO future_posts(USER_ID, CAPTION, TAGGED_USERS, LOCATION, POST_DT, POST_TO_INSTA, POST_TO_TIKTOK, POST_TO_YOUTUBE) " +
                "VALUES(:id, :caption, :taggedUsers, :location, :postDT, :postToInsta, :postToTiktok, FALSE) RETURNING ID"
    )
